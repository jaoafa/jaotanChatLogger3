...
import {Guild, Message, NewsChannel, TextChannel, ThreadChannel, User} from "discord.js";
...
export async function check(conn: mysql.Connection, message: Message) {
  ...
  await checkExistsChannel(conn, message.channel.isThread() ? message.channel.parent : message.channel)
...
export async function checkExistsGuild(conn: mysql.Connection, guild: Guild | null) {
  if (guild === null) {
    throw new Error("guild is null.")
  }
  const [rows] = (await conn.execute("SELECT * FROM guilds WHERE guild_id = ?", [
    guild?.id,
  ])) as RowDataPacket[][]
  if (rows.length !== 0) {
    // 既にサーバ情報がDBにある
...
export async function checkExistsChannel(
  ...
  const [rows] = (await conn.execute("SELECT * FROM channels WHERE channel_id = ?", [
    channel?.id,
  ])) as RowDataPacket[][]
  if (rows.length !== 0) {
    // 既にサーバ情報がDBにある
...
export async function checkExistsThread(conn: mysql.Connection, thread: ThreadChannel | null) {
  if (thread === null) {
    throw new Error("thread is null.")
  }
  const [rows] = (await conn.execute("SELECT * FROM threads WHERE thread_id = ?", [
    thread?.id,
  ])) as RowDataPacket[][]
  if (rows.length !== 0) {
    // 既にサーバ情報がDBにある
...
export async function checkExistsUser(conn: mysql.Connection, author: User) {
  ...
  await conn.execute(
      ...
      [author.id, author.username, author.discriminator, author.bot, author.system]
  )
  await conn.commit()
}